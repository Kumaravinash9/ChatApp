...
},
{
    ...
    if(i==0)
     ...
     pickedFile = await ImagePicker().getVideo(source: ImageSource.gallery);
      ...
      if (pickedFile!= null) {
        ...
        ref=FirebaseStorage.instance.ref().child("${FirebaseAuth.instance.currentUser.uid}/videos").child("${DateTime.now()}.mp4");
      ...
      Scaffold.of(context).showSnackBar(SnackBar(content: Text("Please choose Pick a file",style: TextStyle(
        fontFamily: "Roboto"
      ),),));
      }
  ...
  Widget build(BuildContext context) {
    ...
    child:GridView.builder(gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(childAspectRatio: 3/2,maxCrossAxisExtent: 200,crossAxisSpacing: 20,mainAxisSpacing: 20,
     ...
     return InkWell(
       onTap: (){
         getImage(i);
          ...
          child: Column(
        children: [
          CircleAvatar(child:Icon( icons[i]["icon"]),),
          Text(icons[i]["text"])
        ],
          ),
       ),
