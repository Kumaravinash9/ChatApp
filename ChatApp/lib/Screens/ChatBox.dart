...
{  Timestamp _time=Timestamp.now();
  ...
  FirebaseFirestore.instance.collection('chat').add({
    ...
    DocumentReference docRef= FirebaseFirestore.instance.collection('user').doc(FirebaseAuth.instance.currentUser.uid).collection("friends").doc(widget.userId);
   ...
   "chats":FieldValue.arrayUnion(chats)
    ...
    DocumentReference docRef= FirebaseFirestore.instance.collection('user').doc(widget.userId).collection("friends").doc(FirebaseAuth.instance.currentUser.uid);
    DocumentSnapshot ref= await docRef.get();
    print(ref.data());
    chats=  ref.data()['chats'];
    chats.add({"id":value.id,"timestamp":_time});
    ...
    return Scaffold(
      ...
      widget.url.length>0? CircleAvatar(child: SizedBox(),backgroundImage:NetworkImage(widget.url),radius: 20,) :CircleAvatar(child:Text(widget.username.substring(0,1)),radius: 20,backgroundColor: Colors.green,),
          ...
          body: 
              ...
              children: [
                  ...
                  Expanded(
                     ...
                     child: StreamBuilder(stream: FirebaseFirestore.instance.collection("user").doc(FirebaseAuth.instance.currentUser.uid).collection("friends").doc(widget.userId).snapshots(),builder: (context,snapshot){
                         if (!snapshot.hasData) {
        return Center(
        ...
        { 
         ...
         List<dynamic>sortedmessageBubbles=List.from(messageBubbles.reversed);
          return ListView.builder(reverse: true,itemCount: snapshot.data['chats'].length,itemBuilder: (context,index){
          return StreamBuilder(stream:FirebaseFirestore.instance.collection("chat").doc(sortedmessageBubbles[index]['id']).snapshots(),builder: (context,snapshoty){
            if(snapshoty.hasData)
            {
             ...
             Timestamp t = snapshoty.data['timestamp'];
              ...
              return Bubble(snapshoty.data['message'],FirebaseAuth.instance.currentUser.uid==snapshoty.data['userid'],snapshoty.data['fileType'],"${d.hour.toString()}.${d.minute}");
            }
    ...
    children: [
      Expanded(
        child: Container(
          ...
          var  ref;
          ref=FirebaseStorage.instance.ref().child("${FirebaseAuth.instance.currentUser.uid}/images").child("${DateTime.now()}.jpg");
      ...
      Scaffold.of(context).showSnackBar(SnackBar(content: Text("Please choose Pick a file"),));
      }}
